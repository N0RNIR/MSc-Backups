    ...
    def residue_bonds(self, mutant, model):
        ...
        for mutant in os.listdir(self.mutant_path):
            if mutant .endswith('.pdb'):
                table_dict = {"WT Contacts" : [],  mutant.split(':')[1] + " Contacts" : [],  "Contacts Gained/ Lost":  []}
                mutant_contacts = {}
                model_contacts = {}
                print(mutant)
                ...
                chains = cmd.get_chains(mutant.split('.')[0].replace(':', '_') )
                sequences = []
                ...
                for  contact in model_contacts:
                    if model_contacts[contact] > 0:
                        table_dict.setdefault('WT Contacts', []).append(contact + ': ' + str(model_contacts[contact] ))
                for  contact2 in mutant_contacts:
                    if mutant_contacts[contact2] > 0:
                        table_dict.setdefault(mutant.split(':')[1] + " Contacts", []).append(contact2 + ': ' + str(mutant_contacts[contact2] ))
                data_file = open('/home/rotan/Desktop/Objective2/Obj.P1_2output/' + str(mutant.split('.')[0].replace(':', '_')) + '_contacts.txt', 'w')
                data_file.write(tabulate(table_dict, headers="keys"))
                data_file.close()
                
                
    
    def foldx_stability(self, pdb_path, model_path, model):
        self.pdb_path = pdb_path
        self.model_path = model_path
        self.model = model
        model_foldx_stability = '/home/rotan/Desktop/FoldX/foldx --command=Stability  --pdb-dir=' + model_path + ' --pdb=' + model + ' --output-dir=/home/rotan/Desktop/Objective2/Obj.P2_output' 
        os.system(model_foldx_stability)
        os.chdir(self.pdb_path)
        for file in os.listdir(self.pdb_path):
                if file.endswith('.pdb'):
                    foldx_stability = '/home/rotan/Desktop/FoldX/foldx --command=Stability  --pdb=' + file  + ' --output-dir=/home/rotan/Desktop/Objective2/Obj.P2_output' 
                    os.system(foldx_stability)

p = MutantAnalysis()
p.foldx_stability('/home/rotan/Desktop/Masters/mutant_pdbs2', '/home/rotan/Desktop/', 's_test.pdb')
